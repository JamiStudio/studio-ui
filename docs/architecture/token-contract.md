# Token Contract

Status: Foundation contract
Last updated: 2026-06-09

## Purpose

`packages/tokens` now owns the first machine-readable Studio UI token contract
foundation and deterministic generated output surface. This is not the full token
compiler or workbench editor. It is the checked contract spine that later generation,
workbench editing, registry theme items, and renderer theme references must preserve.

## Contract Source

- Schema: `packages/tokens/schemas/token-set.schema.json`
- Valid fixture: `packages/tokens/fixtures/valid/jami-factory.tokens.json`
- Negative fixtures: `packages/tokens/fixtures/invalid/`
- Generated artifacts: `packages/tokens/generated/`
- Generate command: `pnpm contracts:generate`
- Check command: `pnpm contracts:check`

The fixture follows the DTCG-shaped `$type` / `$value` model and records the
selected source-lock posture in `$extensions["studio-ui"]`: schema version,
DTCG report, migration metadata, and generated output targets.

## Required Token Families

The foundation check requires these early surfaces:

- `color.brand.accent`, anchored on `#C14D84`.
- warm neutral light/dark background and foreground tokens.
- semantic light and dark background, foreground, and accent references.
- typography, spacing, radius, shadow, motion, density, shell, and component-state
  examples.
- deprecated alias coverage through `$deprecated`.
- composite token coverage through the typography fixture.

## Validation Behavior

`pnpm contracts:check` currently enforces:

- schema URL and Studio UI extension metadata.
- schema version and migration metadata.
- generated output target declarations for CSS variables, Tailwind theme variables,
  TypeScript token maps, and shadcn `cssVars`.
- required semantic light/dark tokens.
- token reference resolution.
- foreground/background contrast at 4.5:1 for the initial semantic pairs.
- deterministic generated CSS variables, Tailwind `@theme` variables, TypeScript
  token names and CSS variable references, and shadcn `cssVars` output.
- negative fixtures for broken references and contrast failures.

The check is intentionally local and dependency-free until the compiler package chooses
its runtime validation library.

## Generated Outputs

The current generated outputs are source-fixture artifacts, not a published package
API:

- `packages/tokens/generated/jami.css`
- `packages/tokens/generated/jami.tailwind.css`
- `packages/tokens/generated/jami-tokens.ts`
- `packages/tokens/generated/jami.shadcn.json`
- `packages/tokens/generated/jami-token-provenance.json`

They are regenerated by `pnpm contracts:generate` and drift-checked by
`pnpm contracts:check`, package tests, and `pnpm verify`.
The provenance manifest records the source token fixture hash and the hash of every
generated token output embedded by the `jami-theme` registry item. It deliberately
keeps hosted registry and package publish claims false until those routes are
executed in later groups.

## Not Yet Claimed

This pass does not generate workbench control schemas from every token family or claim
hosted/package publication. Those outputs must be generated mechanically from this
token source in later passes and then kept in the same drift-check loop.
