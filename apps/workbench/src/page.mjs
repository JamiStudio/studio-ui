...
<aside class="ju-workbench" aria-label="Always-live workbench overlay">
  ...
  <div class="ju-dock" aria-label="Workbench panels">
    <details class="ju-dock-panel" open><summary>Theme</summary><div class="ju-dock-body">
      ...
      <div class="ju-control"><span>Backend</span><code id="ju-wb-backend">${escapeHtml(data.workbenchBackendConfig.status)} / ${escapeHtml(data.workbenchBackendConfig.reason ?? "unverified endpoint")}</code></div>
    </div></details>
...
<footer class="ju-footer">
  ...
  <p>Pending: external hosted runtime, hosted persistence, backend registration, and harness runtime — none are claimed here. Backend config evidence: <code>${escapeHtml(data.workbenchBackendConfig.status)}</code>${data.workbenchBackendConfig.reason ? ` / <code>${escapeHtml(data.workbenchBackendConfig.reason)}</code>` : ""}.</p>
</footer>`;
}
...
<body>
${withFocus}
<script>${buildWorkbenchClientScript(brandOptionMap(data.brandOptions), data.workbenchBackendConfig)}</script>
</body>
</html>
`;
}
